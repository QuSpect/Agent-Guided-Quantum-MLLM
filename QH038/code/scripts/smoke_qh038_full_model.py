#!/usr/bin/env python3
"""Two-step Qwen3.8-27B smoke for QH038 quantum or classical arm."""
...
from quantum_qwen38.qh038_shifted_ring_fcvqc_ffn_replacement import QH038Config,install_qh038,unique_trainable_parameters
def main():
 p=argparse.ArgumentParser();p.add_argument("--branch",choices=["quantum","classical"],required=True);a=p.parse_args();torch.manual_seed(20260866);torch.cuda.manual_seed_all(20260866)
 ...
 model=AutoModelForMultimodalLM.from_pretrained(model_dir,local_files_only=True,dtype=torch.bfloat16,device_map="balanced",low_cpu_mem_usage=True,attn_implementation="sdpa");model.eval();model.config.use_cache=False;cfg=QH038Config();replacement,original,audit=install_qh038(model,cfg,branch=a.branch);params=list(unique_trainable_parameters(replacement));opt=torch.optim.AdamW(params,lr=1e-3);rows=[];before=int(replacement.core.circuit_calls.item())
 for step in range(2):
  x=tokens[(7002+step)*64:(7003+step)*64].to(model.device).unsqueeze(0);model.model.language_model.layers[cfg.target_layer].mlp=original
 ...
 calls=int(replacement.core.circuit_calls.item())-before;expected=4 if a.branch=="quantum" else 0
