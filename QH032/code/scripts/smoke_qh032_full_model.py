#!/usr/bin/env python3
"""Two-step train-only QH032 integration smoke on frozen Qwen3.8-27B."""
...
OUTPUT = PROJECT / "artifacts/qh032-full-model-smoke.json"
...
def main() -> None:
    if not torch.cuda.is_available():
        raise RuntimeError("QH032 full-model smoke is GPU-only")
    torch.manual_seed(20260850)
    torch.cuda.manual_seed_all(20260850)
    ...
    config = QH032Config()
    replacements, originals, audit = install_qh032(model, config, branch="quantum")
    ...
    for step, index in enumerate(indices, start=1):
        start = index * length
        input_ids = tokens[start : start + length].to(model.device, dtype=torch.long).unsqueeze(0)
        set_projections(model, originals)
        with torch.inference_mode():
            teacher = backbone(input_ids=input_ids, use_cache=False, return_dict=True).last_hidden_state.detach()
        set_projections(model, replacements)
        optimizer.zero_grad(set_to_none=True)
        started = time.perf_counter()
        student = backbone(input_ids=input_ids, use_cache=False, return_dict=True).last_hidden_state
        denominator = teacher.float().square().mean().clamp_min(1.0e-8)
        loss = (student.float() - teacher.float()).square().mean() / denominator
        loss.backward()
        alpha_grad = float(core.alpha.grad.float().norm())
        theta_grad = float(core.theta.grad.float().norm())
        gamma_grad = float(torch.stack([module.gamma.grad.float().abs() for module in replacements.values()]).norm())
        ...
        rows.append({
            ...
            "alpha_gradient_norm": alpha_grad,
            "theta_gradient_norm": theta_grad,
            "gamma_gradient_norm": gamma_grad,
    ...
    if rows[0]["alpha_gradient_norm"] <= 0.0:
        raise AssertionError("alpha did not receive the identity-initialization gradient")
    if rows[1]["theta_gradient_norm"] <= 0.0:
        raise AssertionError("theta did not receive gradient after alpha activation")
    frozen_grad_tensors = sum(
        int(parameter.grad is not None) for parameter in model.parameters() if not parameter.requires_grad
    )
    ...
    payload = {
        ...
        "final_gammas": {str(index): float(module.gamma.detach()) for index, module in replacements.items()},
        "unique_trainable_tensor_count": len(trainable),
        "unique_trainable_parameter_count": sum(parameter.numel() for parameter in trainable),
