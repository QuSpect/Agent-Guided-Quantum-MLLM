...
from quantum_qwen38.residual_fidelity_router import ResidualFidelityRelationAdapter


def run(kind: str, config: QH017Config) -> dict:
    module = ResidualFidelityRelationAdapter(config, residual_kind=kind).cuda()
    for name in (
        "token_down",
        "object_down",
        "text_down",
    ...
    def objective() -> torch.Tensor:
        return (module.residual(x, shapes).float() * probe).sum()
    ...
    target_name = (
        "router.residual.affine"
        if kind == "classical"
        else "router.residual.theta"
    )
...
def main() -> None:
    if not torch.cuda.is_available():
        raise RuntimeError("QH-018 test requires CUDA")
    ...
    cpu_rejected = False
    try:
        cpu_module = ResidualFidelityRelationAdapter(config)
    ...
    artifact = {
        "status": "pass"
        if all((equal_parameters, gradients_ok, finite_difference_ok, routing_ok, cpu_rejected))
        else "fail",
    ...
    output = Path("artifacts/qh018-gpu-validation.json")
