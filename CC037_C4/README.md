...
本目录对应 EXP-044（CC037 高压缩完整 FFN 替代的 C4-256 独立确认）。代码副本可在任意新目录使用；模型、数据、缓存和输出均采用相对路径，不需要原服务器。模型结构、数据划分、seed、训练预算和评价定义保持历史登记值，只有路径与缓存位置被改为可移植写法。
...
├─ models/CC037_C4/      # 从对应 Hugging Face 参数包取得
├─ datasets/                    # 下载和预处理结果
...
从对应 `CC037_C4` Hugging Face 参数包取得整个 `models/CC037_C4/`，保持相同相对层级：

```text
./models/CC037_C4/checkpoints/...
```

本 GitHub 代码包不重复提交大权重，也不捏造尚未发布的公网方案 URL。发布到组织账号后，可把实际仓库 ID 代入：

```bash
hf download YOUR_ORG/CC037_C4 --include "models/CC037_C4/**" --local-dir .
```
...
`manifest.json` 同时保存原始哈希和路径可移植副本哈希。性能事实与档位见 `../../reports/CC037_C4.md`。
