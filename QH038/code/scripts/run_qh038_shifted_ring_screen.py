#!/usr/bin/env python3
"""Run locked base/zero/QH038/CC038 train-only arms."""
...
PROJECT=Path(__file__).resolve().parents[2];LOCK=PROJECT/"artifacts/qh038-shifted-ring-screen-lock.json";TOKENS=PROJECT/"datasets/processed/wikitext2_qwen38/train-tokens.pt";SOURCE=PROJECT/"code/src/quantum_qwen38/qh038_shifted_ring_fcvqc_ffn_replacement.py";ENTRY=PROJECT/"code/scripts/run_qh038_shifted_ring_screen.py";OUT=PROJECT/"artifacts/qh038_shifted_ring_screen";sys.path.insert(0,str(PROJECT/"code/src"))
from quantum_qwen38.qh038_shifted_ring_fcvqc_ffn_replacement import ExactShiftedRingGroupedQVAFCore,QH038Config,install_qh038,unique_trainable_parameters
def sha(p):
 d=hashlib.sha256()
 with p.open("rb") as h:
...
def train(model,replacement,original,tokens,lock,cfg):
 ...
 calls=int(replacement.core.circuit_calls.item())-before;expected=2*len(lock["train_indices"]) if isinstance(replacement.core,ExactShiftedRingGroupedQVAFCore) else 0;frozen=sum(int(p.grad is not None) for p in model.parameters() if not p.requires_grad)
...
def save_checkpoint(replacement,opt,path,candidate,lock,cfg,training,model_dir):
 torch.save({"candidate":candidate,"config":cfg.__dict__,"data_lock_sha256":sha(LOCK),"source_sha256":sha(SOURCE),"entrypoint_sha256":sha(ENTRY),"base_model_path":str(model_dir),"base_config_sha256":sha(model_dir/"config.json"),"steps":training["steps"],"precision":{"base":"bfloat16","simulator":"float32 real exact statevector","device":"CUDA GPU only"},"replacement_state_dict":{k:v.detach().cpu() for k,v in replacement.state_dict().items()},"optimizer":"AdamW","optimizer_state_dict":opt.state_dict(),"scheduler_state_dict":None,"training_summary":{k:v for k,v in training.items() if k!="losses"}},path)
def main():
 p=argparse.ArgumentParser();p.add_argument("--candidate",choices=["base","zero","qh038","cc038"],required=True);a=p.parse_args();lock=json.loads(LOCK.read_text())
 if lock["status"]!="locked_before_qh038_train_only_screen_results":raise RuntimeError("invalid lock")
 random.seed(lock["seed"]);np.random.seed(lock["seed"]);torch.manual_seed(lock["seed"]);torch.cuda.manual_seed_all(lock["seed"]);tokens=torch.load(TOKENS,map_location="cpu",weights_only=True);model_dir=Path((PROJECT/"records/active_model_path.txt").read_text().strip());model=AutoModelForMultimodalLM.from_pretrained(model_dir,local_files_only=True,dtype=torch.bfloat16,device_map="balanced",low_cpu_mem_usage=True,attn_implementation="sdpa");model.eval();model.config.use_cache=False;cfg=QH038Config();replacement=original=opt=None
 if a.candidate=="base":model.requires_grad_(False);audit={"candidate":"base","net_model_parameter_reduction":0};training={"steps":0}
 else:
  branch="classical" if a.candidate=="cc038" else "quantum";replacement,original,audit=install_qh038(model,cfg,branch=branch)
 ...
 if a.candidate in {"qh038","cc038"}:save_checkpoint(replacement,opt,OUT/f"{a.candidate}-screen-checkpoint.pt",a.candidate,lock,cfg,training,model_dir)
