...
本目录对应 EXP-015（QH-017 量子态保真度关系路由）。代码副本可在任意新目录使用；模型、数据、缓存和输出均采用相对路径，不需要原服务器。模型结构、数据划分、seed、训练预算和评价定义保持历史登记值，只有路径与缓存位置被改为可移植写法。
...
├─ models/QH017/      # 从对应 Hugging Face 参数包取得
├─ datasets/                    # 下载和预处理结果
...
从对应 `QH017` Hugging Face 参数包取得整个 `models/QH017/`，保持相同相对层级：

```text
./models/QH017/checkpoints/...
```

本 GitHub 代码包不重复提交大权重，也不捏造尚未发布的公网方案 URL。发布到组织账号后，可把实际仓库 ID 代入：

```bash
hf download YOUR_ORG/QH017 --include "models/QH017/**" --local-dir .
```
...
登记入口：`test_qh017_gpu.py`。失败、停止或纯审计方案只复现实际达到的 static/smoke/训练/冻结评价阶段；不会把未运行的自由生成伪装为已支持。成功或部分成功方案沿用登记的完整模型前向和真实数据冻结推理。

`run_reached_stage.py` 读取 `stage_plan.json`，依次执行下列展开命令：

```bash
python code/scripts/prepare_clevr_relations.py
python code/scripts/test_qh017_gpu.py
```
...
| 主要源码 |
| --- |
| `code/src/quantum_qwen38/fidelity_relation_router.py` |

| 原实验入口 |
| --- |
| `code/scripts/prepare_clevr_relations.py` |
| `code/scripts/test_qh017_gpu.py` |

`manifest.json` 同时保存原始哈希和路径可移植副本哈希。性能事实与档位见 `../../reports/QH017.md`。
