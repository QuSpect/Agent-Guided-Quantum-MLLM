#!/usr/bin/env python3
"""Run locked base/shared/QH034/CC034 train-only screening arms."""
...
PROJECT=Path(__file__).resolve().parents[2]; LOCK=PROJECT/"artifacts/qh034-spectral-screen-lock.json"
TOKENS=PROJECT/"datasets/processed/wikitext2_qwen38/train-tokens.pt"; OUT=PROJECT/"artifacts/qh034_spectral_screen"
sys.path.insert(0,str(PROJECT/"code/src"))
from quantum_qwen38.qh034_spectral_modulation_replacement import ExactQuantumSpectralCore,QH034Config,install_qh034,unique_trainable_parameters
def sha256(path):
    d=hashlib.sha256()
...
def args():
    p=argparse.ArgumentParser(); p.add_argument("--candidate",choices=["base","sharedsvd1024","qh034","cc034"],required=True); return p.parse_args()
...
def train(model,replacements,originals,tokens,lock):
    ...
    cfg=QH034Config(); core=replacements[cfg.layer_indices[0]].core; before=int(core.circuit_calls.item())
    losses=[]; seconds=[]; theta=[]; gamma=[]
    for pos,i in enumerate(lock["train_indices"],1):
        x=block(tokens,int(i),lock["sequence_length"],model.device); set_proj(model,originals)
        ...
        loss.backward(); theta.append(float(core.theta.grad.norm())); gamma.append(float(torch.stack([m.gamma.grad.abs() for m in replacements.values()]).norm()))
    ...
    calls=int(core.circuit_calls.item())-before; expected=len(lock["train_indices"])*2 if isinstance(core,ExactQuantumSpectralCore) else 0
    ...
    return {"steps":len(losses),"kl_first32":float(np.mean(losses[:32])),"kl_last32":float(np.mean(losses[-32:])),"kl_min":float(np.min(losses)),
      "theta_grad_mean":float(np.mean(theta)),"gamma_grad_mean":float(np.mean(gamma)),"mean_student_step_seconds":float(np.mean(seconds)),
      "simulator_calls":calls,"frozen_gradient_tensor_count":frozen,"final_theta_norm":float(core.theta.detach().norm()),
      "final_gammas":{str(i):float(m.gamma.detach()) for i,m in replacements.items()},"losses":losses}
def save_ckpt(replacements,path):
    first=sorted(replacements)[0]; torch.save({"core.theta":replacements[first].core.theta.detach().cpu(),
      **{f"gamma.{i}":m.gamma.detach().cpu() for i,m in replacements.items()}},path)
def main():
    a=args(); lock=json.loads(LOCK.read_text())
    if lock["status"]!="locked_before_qh034_train_only_screen_results": raise RuntimeError("invalid lock")
    ...
    if a.candidate=="base": model.requires_grad_(False); audit={"candidate":"base","net_model_parameter_reduction":0}; training={"steps":0}
        ...
        branch="classical" if a.candidate=="cc034" else "quantum"; replacements,originals,audit=install_qh034(model,QH034Config(),branch=branch)
        if a.candidate=="sharedsvd1024":
            for m in replacements.values(): m.branch_enabled=False; m.requires_grad_(False)
            training={"steps":0}
        else: training=train(model,replacements,originals,tokens,lock)
    ...
    if replacements is not None and a.candidate in {"qh034","cc034"}: save_ckpt(replacements,OUT/f"{a.candidate}-screen-checkpoint.pt")
