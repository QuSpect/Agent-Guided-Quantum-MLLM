# QH044_FORMAL：从零复现实验代码

本目录对应 EXP-046（QH-044 冻结检查点 C4-128 正式因果评价）。代码副本可在任意新目录使用；模型、数据、缓存和输出均采用相对路径，不需要原服务器。模型结构、数据划分、seed、训练预算和评价定义保持历史登记值，只有路径与缓存位置被改为可移植写法。
...
├─ models/QH044_FORMAL/      # 从对应 Hugging Face 参数包取得
├─ datasets/                    # 下载和预处理结果
...
从对应 `QH044_FORMAL` Hugging Face 参数包取得整个 `models/QH044_FORMAL/`，保持相同相对层级：

```text
./models/QH044_FORMAL/checkpoints/...
```

本 GitHub 代码包不重复提交大权重，也不捏造尚未发布的公网方案 URL。发布到组织账号后，可把实际仓库 ID 代入：

```bash
hf download YOUR_ORG/QH044_FORMAL --include "models/QH044_FORMAL/**" --local-dir .
```
...
登记入口：`c4_qh044_formal.py`。失败、停止或纯审计方案只复现实际达到的 static/smoke/训练/冻结评价阶段；不会把未运行的自由生成伪装为已支持。成功或部分成功方案沿用登记的完整模型前向和真实数据冻结推理。
...
python code/scripts/c4_qh044_formal.py run --arm base
python code/scripts/c4_qh044_formal.py run --arm zero
python code/scripts/c4_qh044_formal.py run --arm qh044
python code/scripts/c4_qh044_formal.py run --arm cc044
python code/scripts/c4_qh044_formal.py run --arm qh044_noent
python code/scripts/c4_qh044_formal.py run --arm qh044_dq
python code/scripts/c4_qh044_formal.py analyze
```
...
| 主要源码 |
| --- |
| `code/src/quantum_qwen38/qh037_grouped_qvaf_ffn_replacement.py` |

| 原实验入口 |
| --- |
| `code/scripts/c4_qh044_formal.py` |
| `code/scripts/prepare_c4_validation_shard.py` |
| `code/scripts/prepare_wikitext.py` |
| `code/scripts/qh044_extended_distillation.py` |

`manifest.json` 同时保存原始哈希和路径可移植副本哈希。性能事实与档位见 `../../reports/QH044_FORMAL.md`。
