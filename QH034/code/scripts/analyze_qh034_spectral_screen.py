#!/usr/bin/env python3
"""Analyze the locked QH034 train-only spectral screen."""
from __future__ import annotations
import hashlib,json
from datetime import datetime,timezone
from pathlib import Path
import numpy as np
ROOT=Path(__file__).resolve().parents[1]; RUN=ROOT/"artifacts/qh034_spectral_screen"; LOCK=ROOT/"artifacts/qh034-spectral-screen-lock.json"
FILES={n:RUN/f"screen-{n}-s20260859-n256-e64.json" for n in ("base","sharedsvd1024","qh034","cc034")}; OUT=RUN/"qh034-spectral-screen-analysis.json"
def sha(path):
 d=hashlib.sha256()
...
def main():
 ...
 for label,l,r in (("shared_minus_base","sharedsvd1024","base"),("qh_minus_shared","qh034","sharedsvd1024"),("qh_minus_cc","qh034","cc034")):
 ...
 result={"status":"ok","analysis":"QH034 train-only quantum spectral modulation screen",
   "verdict":"continue_to_new_c4_lock" if gates["continue_to_c4"] else "reject_or_mutate_before_c4",
   "point_nll":{n:float(a.mean()) for n,a in arrays.items()},"comparisons":comparisons,"screen_gates":gates,
   "training":{n:{k:v for k,v in payloads[n]["training"].items() if k!="losses"} for n in ("qh034","cc034")},
   "claim_limit":"Train-only screen; no quality or quantum claim.","validation_rows_inspected":0,"test_rows_inspected":0,
