# QH008_SCIENCEQA：从零复现实验代码

本目录对应 EXP-005（ScienceQA train-only 硬样本挖掘）。代码副本可在任意新目录使用；模型、数据、缓存和输出均采用相对路径，不需要原服务器。模型结构、数据划分、seed、训练预算和评价定义保持历史登记值，只有路径与缓存位置被改为可移植写法。
...
├─ models/QH008_SCIENCEQA/      # 从对应 Hugging Face 参数包取得
├─ datasets/                    # 下载和预处理结果
...
从对应 `QH008_SCIENCEQA` Hugging Face 参数包取得整个 `models/QH008_SCIENCEQA/`，保持相同相对层级：

```text
./models/QH008_SCIENCEQA/checkpoints/...
```

本 GitHub 代码包不重复提交大权重，也不捏造尚未发布的公网方案 URL。发布到组织账号后，可把实际仓库 ID 代入：

```bash
hf download YOUR_ORG/QH008_SCIENCEQA --include "models/QH008_SCIENCEQA/**" --local-dir .
```
...
python code/scripts/run_scienceqa_stage_a.py --candidate qh008
python code/scripts/run_scienceqa_stage_a.py --candidate cc008
python code/scripts/run_scienceqa_stage_a.py --candidate qh008_noent
...
| `code/scripts/mine_scienceqa_hard_train.py` |
| `code/scripts/run_scienceqa_stage_a.py` |
| `code/scripts/test_qh008_gpu.py` |

`manifest.json` 同时保存原始哈希和路径可移植副本哈希。性能事实与档位见 `../../reports/QH008_SCIENCEQA.md`。
