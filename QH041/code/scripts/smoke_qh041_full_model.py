...
from quantum_qwen38.qh041_signed_x_readout_residual import QH041Config,install_qh041,unique_trainable_parameters
def main():
 p=argparse.ArgumentParser();p.add_argument("--branch",choices=["quantum","classical"],required=True);a=p.parse_args();torch.manual_seed(20260872);torch.cuda.manual_seed_all(20260872);model_dir=Path((PROJECT/"records/active_model_path.txt").read_text().strip());tokens=torch.load(PROJECT/"datasets/processed/wikitext2_qwen38/train-tokens.pt",map_location="cpu",weights_only=True);model=AutoModelForMultimodalLM.from_pretrained(model_dir,local_files_only=True,dtype=torch.bfloat16,device_map="balanced",low_cpu_mem_usage=True,attn_implementation="sdpa");model.eval();model.config.use_cache=False;cfg=QH041Config();rep,original,audit=install_qh041(model,BACKBONE,cfg,a.branch);params=list(unique_trainable_parameters(rep));opt=torch.optim.AdamW(params,lr=3e-3);rows=[];before=int(rep.core.circuit_calls.item())
 for step in range(2):
  x=tokens[(7008+step)*64:(7009+step)*64].to(model.device).unsqueeze(0);model.model.language_model.layers[cfg.target_layer].mlp=original
