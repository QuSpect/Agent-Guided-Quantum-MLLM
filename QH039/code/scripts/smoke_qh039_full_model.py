#!/usr/bin/env python3
"""Two-step Qwen3.8-27B QH039 smoke from the registered CC037 scaffold."""
...
def main():
 p=argparse.ArgumentParser();p.add_argument("--branch",choices=["quantum","classical"],required=True);a=p.parse_args();torch.manual_seed(20260868);torch.cuda.manual_seed_all(20260868);model_dir=Path((PROJECT/"records/active_model_path.txt").read_text().strip());tokens=torch.load(PROJECT/"datasets/processed/wikitext2_qwen38/train-tokens.pt",map_location="cpu",weights_only=True)
 model=AutoModelForMultimodalLM.from_pretrained(model_dir,local_files_only=True,dtype=torch.bfloat16,device_map="balanced",low_cpu_mem_usage=True,attn_implementation="sdpa");model.eval();model.config.use_cache=False;cfg=QH039Config();rep,original,audit=install_qh039(model,BACKBONE,cfg,a.branch);params=list(unique_trainable_parameters(rep));opt=torch.optim.AdamW(params,lr=3e-3);rows=[];before=int(rep.core.circuit_calls.item())
 for step in range(2):
  x=tokens[(7004+step)*64:(7005+step)*64].to(model.device).unsqueeze(0);model.model.language_model.layers[cfg.target_layer].mlp=original
  with torch.inference_mode():teacher=model(input_ids=x,use_cache=False).logits[:,:-1].detach()
  model.model.language_model.layers[cfg.target_layer].mlp=rep;opt.zero_grad(set_to_none=True);started=time.perf_counter();student=model(input_ids=x,use_cache=False).logits[:,:-1];loss=F.kl_div(F.log_softmax(student.float(),-1),F.softmax(teacher.float(),-1),reduction="sum")/(student.shape[0]*student.shape[1]);loss.backward();rows.append({"step":step,"kl":float(loss),"theta_nonzero":int((rep.core.theta.grad.abs()>0).sum()),"alpha_nonzero":int((rep.alpha.grad.abs()>0).sum()),"frozen_gradient_tensor_count":sum(int(p.grad is not None) for p in model.parameters() if not p.requires_grad)});torch.nn.utils.clip_grad_norm_(params,1.0);opt.step();torch.cuda.synchronize();rows[-1]["seconds"]=time.perf_counter()-started
