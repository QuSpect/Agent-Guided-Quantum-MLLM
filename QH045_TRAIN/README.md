# QH045_TRAIN：从零复现实验代码

本目录对应 EXP-047（QH-045 同起点 64 参数可选择纠缠）。代码副本可在任意新目录使用；模型、数据、缓存和输出均采用相对路径，不需要原服务器。模型结构、数据划分、seed、训练预算和评价定义保持历史登记值，只有路径与缓存位置被改为可移植写法。
...
├─ models/QH045_TRAIN/      # 从对应 Hugging Face 参数包取得
├─ datasets/                    # 下载和预处理结果
...
从对应 `QH045_TRAIN` Hugging Face 参数包取得整个 `models/QH045_TRAIN/`，保持相同相对层级：

```text
./models/QH045_TRAIN/checkpoints/...
```

本 GitHub 代码包不重复提交大权重，也不捏造尚未发布的公网方案 URL。发布到组织账号后，可把实际仓库 ID 代入：

```bash
hf download YOUR_ORG/QH045_TRAIN --include "models/QH045_TRAIN/**" --local-dir .
```
...
登记入口：`qh045_selective_entanglement_screen.py`。失败、停止或纯审计方案只复现实际达到的 static/smoke/训练/冻结评价阶段；不会把未运行的自由生成伪装为已支持。成功或部分成功方案沿用登记的完整模型前向和真实数据冻结推理。

`run_reached_stage.py` 读取 `stage_plan.json`，依次执行下列展开命令：

```bash
python code/scripts/prepare_wikitext.py
python code/scripts/qh045_selective_entanglement_screen.py --candidate base
python code/scripts/qh045_selective_entanglement_screen.py --candidate noent
python code/scripts/qh045_selective_entanglement_screen.py --candidate qh045
python code/scripts/qh045_selective_entanglement_screen.py --candidate cc045
```
...
| 主要源码 |
| --- |
| `code/src/quantum_qwen38/qh045_selective_entanglement_ffn_replacement.py` |

| 原实验入口 |
| --- |
| `code/scripts/prepare_wikitext.py` |
| `code/scripts/qh045_selective_entanglement_screen.py` |

`manifest.json` 同时保存原始哈希和路径可移植副本哈希。性能事实与档位见 `../../reports/QH045_TRAIN.md`。
