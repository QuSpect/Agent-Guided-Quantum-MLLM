...
def main() -> None:
    ...
    module = QuantumResidualAdapter(config).to(device=device, dtype=torch.bfloat16)
    ...
    result = {
        ...
        "relative_delta_rms": float(delta.square().mean().sqrt() / hidden.detach().float().square().mean().sqrt()),
